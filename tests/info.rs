...
#[actix_rt::test]
async fn valid_token_valid_account_name_in_blockchain() {
...
#[actix_rt::test]
async fn valid_token_valid_account_name_not_in_blockchain() {
	let settings = Settings::new().unwrap();
    ...
    let request_url = "http://127.0.0.1:8080/api/v1/info/".to_owned() + settings.testing.invalid_backend_id.as_str();
    let request = ureq::post(request_url.as_str())
        .send_json(ureq::json!({
            "account_name": settings.testing.account_name,
            "token": authentication.get("token").unwrap()
...
#[actix_rt::test]
async fn valid_token_invalid_account_name_in_blockchain() {
	let settings = Settings::new().unwrap();
...
#[actix_rt::test]
async fn invalid_token_valid_account_name_in_blockchain() {
	let settings = Settings::new().unwrap();
...
#[actix_rt::test]
async fn invalid_token_invalid_account_name_in_blockchain() {
	let settings = Settings::new().unwrap();
